...
- **🧹 Code Hygiene & Local Verification**: Pruned dead imports (`dart:js`, `shared_preferences`, `notification_service`), resolved missing dependency for `dart_quill_delta`, declared `events` compound index in `firestore.indexes.json`, and added a one-click local `check.bat` test runner.
